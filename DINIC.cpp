...
public:
	...
	vector< resEdge > bfs(){
		...
		if(!achei) return vector< resEdge >();
	...
	void rebuildRes(){
		...
		for(int i = 0; i<G.size(); i++){
			...
			for(edge v : G[i]){
				if(v.c - v.f) R[i].push_back(resEdge(v.id, v.c - v.f, true));
				if(v.f) R[v.id].push_back(resEdge(i, v.f, false));
			}
	...
	Dinic(const vector< vector< pair< int, int > > > &H, int s_, int t_){
		...
		while(1){
			...
			int flow = 1e8;
			for(resEdge e : path) flow = min(flow, e.c);

			int v = s;
			for(resEdge u : path){
				for(int i = 0; i<G[v].size(); i++){
					if(G[v][i].id == u.id){
						if(u.good) G[v][i].f += flow;
						else G[v][i].f -= flow;
						break;
					}
				}
