...
template <typename T> struct Matrix : std::vector<std::vector<T>> {
  T neutral;
  int rows;
  int cols;
  ...
  Matrix(int _rows, int _cols, T _neutral = 0)
      : std::vector<std::vector<T>>(_rows, std::vector<T>(_cols, _neutral)) {
    SetDims(_neutral);
  }
  ...
  Matrix<T> operator-(const Matrix<T> &M) { return (*this) + M.neg(); }
...
template <typename T> struct SquareMatrix : Matrix<T> {
...
int main() {
  ...
  Matrix<double> B = {{1.0}, {0.0}, {1.0}, {0.0}};
