...
template <int n_rows, int n_cols, typename T> class Matrix {
public:
  ...
  void set_get(std::function<T(int, int)> _get) { get = _get; }
...
public:
  std::function<T(int, int)> default_get = [&](int i, int j) {
    return data[i][j];
  };

  ExplicitMatrix(T _neutral = 0)
      : Matrix<n_rows, n_cols, T>([&](int i, int j) { return data[i][j]; },
                                  _neutral) {
  ...
  ExplicitMatrix(ExplicitMatrix<n_rows, n_cols, T> &rhs)
      : Matrix<n_rows, n_cols, T>(rhs.neutral) {
    for (int i = 0; i < n_rows; i++)
      for (int j = 0; j < n_cols; j++)
        data[i][j] = rhs.get(i, j);
    this->set_get(default_get);
  ...
  ExplicitMatrix<n_rows, rhs_cols, T>
  operator*(Matrix<n_cols, rhs_cols, T> &rhs) {
  ...
  template <int rhs_cols> void operator*=(Matrix<n_cols, rhs_cols, T> &rhs) {
    *this = (*this) * rhs;
  }
...
public:
  SquareMatrix(T _neutral = 0) : ExplicitMatrix<len, len, T>(_neutral) {}
...
int main() {
  ...
  ExplicitMatrix<2, 2, int> M = {{1, 2}, {3, 4}};
