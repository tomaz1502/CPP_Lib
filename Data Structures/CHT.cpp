...
struct CHT { // queries can be in arbitrary order. insertions must be in decreasing slope
    
    deque< Line > dq;

    ll query(ll x) {
        if(dq.size() == 1) return dq[0].eval(x);
            
        int sz = (int)dq.size();    
        if(dq[sz - 1].intersectX(dq[sz - 2]) <= x) return dq.back().eval(x);
