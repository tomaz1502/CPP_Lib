...
struct segNode {
  ll tag, answer, sum;
  segNode(ll tag_, ll answer_, ll sum_)
      : tag(tag_), answer(answer_), sum(sum_) {}
};
...
struct SegTree { // Array is 0-based, Tree is 1 based
  ...
  void update(ll idL, ll idR, ll value) {
    update(1, 0, sz - 1, idL, idR, value);
  }
...
struct Event {
  ...
  Event(int down_, int up_, int x_, int type_)
      : up(up_), down(down_), x(x_), type(type_) {}
};
...
int main() {
  ...
  vector<Rec> Recs(n);
  for (Rec &R : Recs)
    cin >> R.x1 >> R.y1 >> R.x2 >> R.y2; // bottom left and up right corners
